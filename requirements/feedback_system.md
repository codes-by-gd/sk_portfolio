...
* 💬 **[4. Citizen Feedback Module](./feedback_system.md) (You are here)** — Homepage featured carousels, detailed submission forms, photo uploads, and AJAX paginations.
...
* **Fields Shown:** Citizen Name, Ward Area, Rating (1 to 5 gold stars), Review Message, and the custom **User Avatar Image** uploaded or managed from the admin panel.
...
* **Optional Mobile Photo Capture:** A specialized upload button. On mobile devices, clicking this button must invoke the native camera app so citizens can snap local photos of development works or issues directly.
...
* **Privacy Controls & Avatar Rendering:** To protect citizen privacy, mobile numbers must be omitted in this public list. It must render the citizen's avatar image if present (using standard DaisyUI avatar component layout), and fallback to displaying the citizen's initials if no custom avatar image has been uploaded. It will display the citizen's name, area, feedback title, rating stars, review message, and user-uploaded photos.
* **Interactive Image Lightbox:** Clicking any citizen-uploaded media thumbnail inside the review listing must trigger a native, high-fidelity same-page Lightbox viewer modal for a premium user experience without opening new tabs.
